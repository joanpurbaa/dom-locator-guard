...
export function generateReport(
  ...
  const summary = buildSummary(locatorChanged.length, added.length, removed.length, current.featureName);
...
function buildSummary(changed: number, added: number, removed: number, feature: string): string {
...
export function formatConsole(report: LocatorReport): string {
  ...
  lines.push(`║  Status    : ${report.status === 'clean' ? '✓ Clean' : report.status === 'warning' ? '⚠ Warning' : '✗ Error'}`.padEnd(59) + '║');
  lines.push(`║  Changes   : ${report.changedCount} locator(s)  +${report.addedCount} added  -${report.removedCount} removed`.padEnd(59) + '║');
  lines.push('╠' + '═'.repeat(58) + '╣');

  const locatorChanges = report.diffs.filter(d => d.verdict === 'locator_changed');
  ...
  if (locatorChanges.length === 0) {
    ...
    for (const diff of locatorChanges) {
      ...
      for (const change of diff.locatorChanges) {
        lines.push(`║`);
        lines.push(`║  [${change.severity.toUpperCase()}] Attribute: ${change.attribute}`);
...
export function formatHTML(report: LocatorReport): string {
  ...
  const changesHTML = report.diffs
    ...
    .map(diff => {
      const changesRows = diff.locatorChanges.map(change => `
        <tr>
