...
import { buildSourceLocation } from "./source-locator";
...
export function normalizeElement(
	...
	const sourceLocation: SourceLocation | undefined =
		source !== undefined && (el as any).startIndex !== undefined
			? buildSourceLocation(source, (el as any).startIndex)
			: undefined;
	...
	for (const child of el.children) {
		if (child.type === "tag") {
			...
			if (!SKIP_TAGS.has(childEl.name.toLowerCase())) {
				children.push(normalizeElement(childEl, undefined, source));
			}
		}
	}
...
export function parseHTML(
	...
	const src = source ?? html; // use html itself as source for line tracking
	...
	const handler = new DomHandler(
		(err, dom) => {
			...
			// Fallback: if no body, use first tag element
			function findFirst(nodes: Node[]): Element | null {
				for (const node of nodes) {
					if (node.type === "tag") return node as Element;
				}
				return null;
			}

			const bodyEl = findBody(dom) ?? findFirst(dom);
			if (bodyEl) {
				root = normalizeElement(bodyEl, undefined, src);
			}
		},
